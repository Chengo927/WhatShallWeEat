### 目标
实现 4 个明确能力：1) 点击菜品后跳转到该菜对应的小红书链接；2) 在“我的”页面接入微信云开发登录；3) 给云存储建立清晰的数据边界与落库规范；4) 让云端能够稳定保存菜品名、照片、链接，以及日历里记录过的历史菜品。

### 方案（指定）
- 先补齐“菜品详情跳转链路”：所有可点击菜品统一走一条 `detailtap -> 链接承接页/跳转逻辑`，不要分散在多个页面各写一套。
- 菜品数据模型扩展出明确的链接字段，建议统一为 `xhsLink`，并保留 `img`/图片地址字段，避免后面云端、本地、历史快照各用一套命名。
- 先验证“小程序内直接打开小红书链接”是否可行：若小红书 H5 域名不能直接用于 `web-view` 或业务域名白名单无法通过，必须提供降级方案，优先级为：
  1. 可直接 `web-view` 打开则直开；
  2. 不能直开则进入中间页，展示菜名、封面、链接按钮与“复制链接”按钮；
  3. 最差兜底也要保证用户能复制链接，而不是点了没反应。
- 云端能力采用“本地优先、登录后同步”的策略，不要一上来把所有读写都改成纯云端；先保证离线还能正常用，再补云同步。
- “我的”页先做微信身份登录与云状态展示，不做复杂账号系统；默认使用微信云开发提供的身份能力 + 云函数拿 `openid`。
- 需要把包体积约束作为硬规划写进方案：根据微信开放文档，当前整个小程序所有分包总大小不超过 30M（服务商代开发不超过 20M），单个主包/分包不能超过 2M；因此图片资源不能无限制塞进主包。
- 微信开放文档没有给“单张图片超过多少必须分包”的单独阈值，真正要盯的是包体积预算；所以策略不应写成“某张图超过 X 就拆包”，而应写成“当本地图片资源让主包接近 2M 时，优先把非启动页能力拆分包，或把大图改为云存储/网络资源”。
- 当前 `菜单 / 日历 / 我的` 都是 `tabBar` 页面，而微信开放文档要求 `tabBar` 页面必须在主包内，所以“我的”页即便新增登录，也不能通过把它挪去分包来规避主包限制。
- 云数据拆成 3 类主表：
  1. `users`：记录用户身份与同步状态；
  2. `dish_library`：记录菜品主数据（菜名、封面、链接、分类等）；
  3. `meal_history`：记录某个用户在某一天吃过哪些菜，并保留快照。
- 历史记录不要只存 `dishId[]`，还要存一份 `dishesSnapshot[]`，至少包含 `dishId / name / img / xhsLink`。原因是后续即使菜库改名、删图、换链接，历史仍能完整回放。
- 云同步分两段：
  1. 首次登录：把本地已有的历史菜单合并上云；
  2. 后续修改菜单：本地写成功后再增量同步到云端。
- 第一版先把“历史菜单”作为核心上云对象；候选池、最近抽奖结果、搜索词、当前筛选状态不要强行一起上云，避免第一版范围失控。

### 文件范围
- `app.js`
- `app.json`
- `pages/order/index.js`
- `pages/order/index.wxml`
- `pages/calendar/index.js`
- `pages/calendar/index.wxml`
- `pages/mine/index.js`
- `pages/mine/index.wxml`
- `pages/mine/index.wxss`
- `components/dish-card/dish-card.js`
- `components/dish-card/dish-card.wxml`
- `data/dishes.js`
- `utils/storage.js`
- `utils/cloud.js`（新增，封装云开发初始化、登录态、同步调用）
- `pages/dish-link/index.js`（新增，若当前仓库没有则补回）
- `pages/dish-link/index.json`（新增）
- `pages/dish-link/index.wxml`（新增）
- `pages/dish-link/index.wxss`（新增）
- `cloudfunctions/login/index.js`（新增）
- `cloudfunctions/syncMealHistory/index.js`（新增）
- `cloudfunctions/getMealHistory/index.js`（新增）
- `cloudfunctions/upsertDishLibrary/index.js`（新增，可选；若第一版只用静态菜库可延后）
- `docs/PROJECT_CONTEXT.md`
- `package-history/*` 或类似分包目录（可选；若后续历史详情、链接承接页、云同步说明页要拆分包）

### 约束（必须遵守）
1. **页面配置约束**：保留当前页面已存在的 `renderer` / `componentFramework` / `navigationStyle` 配置；除非本次任务明确要求调整，并写明原因。
2. **WXSS 选择器限制**：避免 tag selector / #id / attribute selector，统一用 class。
3. **链接跳转约束**：不能假设小红书链接一定能在小程序里直接打开；必须先验证 `web-view` / 业务域名能力，不通过时要有可用兜底，不允许“点击后无反馈”。
4. **云存储边界约束**：第一版只做“登录 + 菜库主数据 + 历史菜单同步”；不要把所有本地状态一次性上云。
5. **数据一致性约束**：历史记录必须保留菜品快照，不能只存 `dishId`。
6. **同步策略约束**：先本地成功，再同步云端；云同步失败不能阻断本地核心点菜链路。
7. **包体积约束**：按微信开放文档，整个小程序所有分包总大小不超过 30M（服务商代开发不超过 20M），单个主包/分包不能超过 2M；`tabBar` 页面必须在主包内。图片与静态资源方案必须围绕这个约束设计。
8. **图片资源约束**：不要把大量菜品大图直接塞进仓库主包；主包只保留首屏必需资源、占位图和少量高频缩略图，其余优先考虑云存储/CDN/网络地址，或跟随非 tab 页一起进分包。
9. **版本管理**：建议分支名 `feature/cloud-login-xhs-links`，建议 Commit 规范 `feat: add cloud login and dish xhs sync plan`.

### 验收标准
- 菜单页点击任意已配置小红书链接的菜品时，能进入对应链接链路；若不能直接打开小红书，也至少能进入承接页并复制链接。
- “我的”页能看到明确的登录入口、登录状态、同步状态和最近同步时间。
- 登录后，云端至少能看到 3 类数据：用户信息、菜品主数据、历史菜单记录。
- 历史菜单记录中，单日数据至少包含：日期、菜品列表、每道菜的名称、图片、链接快照。
- 本地已有的历史菜单在首次登录后能同步到云端；换设备后登录同一账号，至少能从云端拉回历史菜单。
- 未登录时，菜单、日历、本地点菜逻辑仍可正常工作；登录失败或云同步失败时不应破坏本地主链路。
- 引入菜品图片、链接承接页、云登录能力后，主包体积仍需控制在 2M 内；若接近限制，必须有明确的分包或云资源迁移方案。
- 若后续新增大量菜品图片，方案中必须说明哪些图片留本地、哪些走云存储，不允许模糊处理。
- 开发者工具和真机上没有新增的空白页、死链、明显报错或长时间无反馈。

### 详细实施步骤
1. **先定数据模型**
- 给 `data/dishes.js` 的每个菜补统一字段：`id`、`name`、`img`、`xhsLink`、`categoryId`、`emoji`。
- 统一约定后续历史快照也使用这套字段，不再混用 `url / recipeUrl / recipeLink / link`。

2. **建立菜品点击跳转链路**
- 在 `components/dish-card` 保持 `detailtap` 事件为唯一入口。
- `pages/order/index` 和后续历史列表都统一监听 `detailtap`。
- 点击后跳去 `pages/dish-link/index`，由承接页判断：
  - 有合法 `xhsLink`：尝试打开；
  - 无链接：展示“暂无链接”；
  - 链接不可直开：展示封面、标题、复制链接按钮、打开说明。

3. **验证小红书打开方式**
- 先在微信开发者工具和真机验证：小红书链接是否允许放进 `web-view`。
- 如果不允许，不要继续赌技术细节，直接走“承接页 + 复制链接”的稳定方案。
- 这一条必须在正式开发前先验证，否则很容易做到一半整条链路推翻。

4. **接入微信云开发基础能力**
- 在 `app.js` 初始化云开发环境。
- 明确 `envId` 配置来源，避免写死在多个文件里。
- 新增 `utils/cloud.js`，统一封装：
  - 云环境初始化
  - 获取登录态
  - 调用云函数
  - 本地缓存最近登录/同步状态

5. **先做包体积与资源预算**
- 先把当前本地图片、未来要新增的小红书封面图、云登录相关代码、历史页资源放进同一个包体积预算表。
- 明确哪些资源必须留主包：
  - `tabBar` 相关页面代码
  - 首页首屏必要资源
  - 通用占位图
- 明确哪些资源不应留主包：
  - 大量菜品原图
  - 历史详情页资源
  - 链接承接页里可延迟加载的封面/说明资源
- 如果菜品图片量继续增长，优先方案是“主包只留 placeholder + 少量缩略图，完整图片走云存储/网络地址”；分包是第二层手段，不是唯一手段。

6. **实现“我的”页登录卡片**
- 当前“我的”页还是占位页，第一版改成：
  - 登录按钮
  - 当前登录状态
  - 最近同步时间
  - 手动同步按钮
  - 云端说明文案
- 登录成功后，在本地缓存最小用户信息，并更新页面状态。

7. **建立云函数和数据库结构**
- `login` 云函数：返回当前用户 `openid`，首次登录时初始化 `users` 文档。
- `syncMealHistory` 云函数：接收本地历史菜单，按 `openid + date` upsert。
- `getMealHistory` 云函数：按 `openid` 拉取历史菜单。
- `dish_library` 可有两种落法：
  - 第一版简单方案：仍以本地 `data/dishes.js` 为主，云端只备份或延后；
  - 更完整方案：增加 `dish_library` 集合，把菜库主数据也落云，便于以后运营维护。
- 若时间有限，优先顺序应为：`users` > `meal_history` > `dish_library`。

8. **规范哪些数据应该上云**
- **第一优先级，必须上云**
  - `users`
    - `openid`
    - `nickname`（若拿不到可先为空）
    - `avatarUrl`（若拿不到可先为空）
    - `createdAt`
    - `lastLoginAt`
    - `lastSyncAt`
  - `dish_library`
    - `dishId`
    - `name`
    - `img`
    - `xhsLink`
    - `categoryId`
    - `emoji`
    - `updatedAt`
  - `meal_history`
    - `openid`
    - `date`
    - `dishIds`
    - `dishesSnapshot`
      - `dishId`
      - `name`
      - `img`
      - `xhsLink`
    - `marks`
    - `source`（`local_migrated` / `cloud_edit`）
    - `updatedAt`
- **第二优先级，可二期再上云**
  - `THINK_POOL`
  - `LAST_LOTTERY_RESULT_YYYY-MM-DD`
  - 用户偏好、分类偏好、是否展示某些引导
- **不建议上云**
  - `searchKeyword`
  - 当前弹窗开关
  - 当前选中分类
  - 抽奖预览结果
  - 纯 UI 状态

9. **定义同步策略**
- 首次登录：
  - 从本地读取 `TODAY_MENU_*`、`MEAL_PLAN_BY_DATE`、`meal_calendar_marks_v1`
  - 生成标准化 `meal_history`
  - 批量推送云端
- 日常使用：
  - 本地点菜/移除成功后，异步调用 `syncMealHistory`
  - 云端失败只提示“同步失败，可稍后重试”，不回滚本地菜单
- 换设备登录：
  - 登录成功后拉取云端 `meal_history`
  - 回填到本地 storage
  - 刷新日历页标记和菜单页状态

10. **历史记录展示建议**
- 第一版不必立刻在“我的”页做完整历史时间线。
- 但至少要保证云端数据结构已经支持以后展示：
  - 某天吃了什么
  - 点击历史菜品还能继续打开对应小红书链接
  - 即使菜库后续变化，历史仍显示当时名称/图片/链接快照

11. **上线前验证顺序**
- 先验证小红书链接链路。
- 再验证“我的”页登录。
- 再验证本地历史首次上云。
- 最后验证新设备拉回历史。
- 每次新增图片或资源后，都要重新检查主包/总包体积，不要只在最终提审前才发现超限。
- 不要反过来做，否则排查时会把“链接问题”“登录问题”“同步问题”混在一起。

### 开发分期建议
- **P1：菜品链接**
  - 只完成 `xhsLink` 字段、点击链路、承接页/兜底页。
- **P2：云登录**
  - 完成 `app.js` 云开发初始化、“我的”页登录状态、`login` 云函数。
- **P3：历史菜单上云**
  - 完成 `meal_history` 数据模型、首次迁移、本地改动后增量同步。
- **P4：云端菜库**
  - 视时间再决定是否把 `dish_library` 从静态文件切换成云端主数据源。

### 页面分包建议清单
| 页面/能力 | 是否必须主包 | 建议处理 | 原因 |
| --- | --- | --- | --- |
| `pages/order/index` | 是 | 留主包 | 是 `tabBar` 页面，且是启动后高频入口 |
| `pages/calendar/index` | 是 | 留主包 | 是 `tabBar` 页面 |
| `pages/mine/index` | 是 | 留主包 | 是 `tabBar` 页面；登录入口也必须跟着留主包 |
| `custom-tab-bar/*` | 是 | 留主包 | `tabBar` 运行依赖 |
| `components/dish-card/*` | 是 | 默认留主包 | 菜单页首屏直接依赖 |
| `utils/storage.js` | 是 | 留主包 | 菜单/日历主链路都会用到 |
| `utils/cloud.js` | 否 | 可先留主包，后续视体积再评估 | 登录按钮在“我的”页里，但逻辑本身不重；先求稳 |
| `pages/dish-link/index` | 否 | 优先候选分包 | 非启动页，点击菜品后才进入 |
| 历史详情页（若新增） | 否 | 放分包 | 低频访问，适合按需加载 |
| 云同步说明页/帮助页（若新增） | 否 | 放分包 | 非核心主链路 |
| 大图预览页（若新增） | 否 | 放分包 | 体积大、访问低频 |

补充说明：
- 这里的核心判断依据不是“能不能分”，而是“是否属于 `tabBar` 主链路”。
- 你的项目当前最稳的拆法不是把“我的”页拆出去，而是把未来新增的低频详情页、历史页、说明页拆出去。

### 图片与资源落地建议
| 资源类型 | 建议存放位置 | 是否进 Git 仓库 | 是否进主包 | 云端字段怎么存 |
| --- | --- | --- | --- | --- |
| 通用占位图 `placeholder` | 本地 `assets` | 是 | 是 | 不需要单独存 |
| 少量高频菜品缩略图 | 本地 `assets` | 是 | 是，但数量要控 | `img` 存本地路径或后续替换成 URL |
| 大量菜品封面图 | 云存储 / CDN / 网络地址 | 否 | 否 | `img` 存 URL |
| 小红书封面图 | 优先云存储 / 网络地址 | 否 | 否 | `img` 或 `coverUrl` 存 URL |
| 用户头像 | 微信返回或云端 URL | 否 | 否 | `avatarUrl` 存 URL |
| 历史记录里的图片 | 不存二进制，只存快照 URL | 否 | 否 | `dishesSnapshot[].img` 存字符串 URL/路径 |

补充说明：
- 历史记录里保存的是“图片地址快照”，不是图片文件本身。
- 如果以后菜品图片越来越多，不要继续往本地 `assets/dishes` 堆；本地包适合留占位图和少量启动即见的缩略图。
- 即使未来保留少量本地缩略图，也要优先压缩成面向列表展示的小图，不要把原图放进仓库。

### 建议上云字段细化
| 集合 | 字段 | 类型 | 说明 |
| --- | --- | --- | --- |
| `users` | `openid` | string | 用户唯一标识 |
| `users` | `nickname` | string | 用户昵称，可为空 |
| `users` | `avatarUrl` | string | 头像 URL，可为空 |
| `users` | `createdAt` | number/date | 首次创建时间 |
| `users` | `lastLoginAt` | number/date | 最近登录时间 |
| `users` | `lastSyncAt` | number/date | 最近同步时间 |
| `dish_library` | `dishId` | string | 菜品 id |
| `dish_library` | `name` | string | 菜名 |
| `dish_library` | `img` | string | 菜品图片 URL 或路径 |
| `dish_library` | `xhsLink` | string | 小红书链接 |
| `dish_library` | `categoryId` | string | 分类 id |
| `dish_library` | `emoji` | string | 分类/菜品 emoji |
| `dish_library` | `updatedAt` | number/date | 最近更新时间 |
| `meal_history` | `openid` | string | 所属用户 |
| `meal_history` | `date` | string | `YYYY-MM-DD` |
| `meal_history` | `dishIds` | string[] | 当天菜品 id 列表 |
| `meal_history` | `marks` | string[] | 日历标记 |
| `meal_history` | `source` | string | `local_migrated` / `cloud_edit` |
| `meal_history` | `updatedAt` | number/date | 最近更新时间 |
| `meal_history` | `dishesSnapshot` | object[] | 历史快照，必须保留 |

`dishesSnapshot` 内建议固定字段：
- `dishId`
- `name`
- `img`
- `xhsLink`
- `categoryId`
- `emoji`

### 包体积检查点
1. 新增 5 张以上本地图片时，立即检查一次主包大小。
2. 新增新页面且不是 `tabBar` 页面时，优先先判断是否应该直接进分包。
3. 引入云登录后，如果“我的”页只是文案和按钮增加，一般不是代码体积风险点；真正容易超的是图片和静态资源。
4. 每次准备提审前，必须检查：
   - 主包是否 < 2M
   - 总包是否 < 30M
   - 是否把不该留主包的图片继续留在主包

### 官方约束记录
- 依据微信开放文档《分包加载》：整个小程序所有分包总大小不超过 30M，服务商代开发不超过 20M；单个主包/分包不能超过 2M。
- 依据微信开放文档《使用分包》：`tabBar` 页面必须在主包内。

### Demo 样例上云方式
你当前 `Demo/demo.md` 的结构是：
- `dish name`：`农家小炒肉`
- `dish photo`：本地文件 `Demo\Weixin Image_20260310204625_15_75.jpg`
- `dish link to redbook`：一段带说明文字的小红书分享文案，里面包含短链接 `http://xhslink.com/o/9XzlPQqCYIN`

对于这种结构，**不要把 `demo.md` 整个文件直接丢进数据库**。正确拆法是：
1. 图片文件上传到**云存储**
2. 菜品元数据写入**云数据库**
3. 小红书链接只保存**清洗后的真实链接字符串**

### 单条 Demo 手动上传步骤
1. 在微信开发者工具里打开云开发环境，确认已经创建好 `envId`。
2. 进入“云开发” -> “存储”。
3. 上传图片 `Demo/Weixin Image_20260310204625_15_75.jpg`。
4. 云存储路径建议统一，例如：
   - `dish-library/dish_001/cover.jpg`
   - 或 `dish-library/nongjiaxiaochaorou/cover.jpg`
5. 上传完成后拿到该图片的 `fileID`。
6. 进入“云开发” -> “数据库”，新建集合 `dish_library`。
7. 新增一条文档，写入类似结构：

```json
{
  "dishId": "dish_001",
  "name": "农家小炒肉",
  "imgFileId": "cloud://<你的环境ID>.xxx/dish-library/dish_001/cover.jpg",
  "img": "",
  "xhsLink": "http://xhslink.com/o/9XzlPQqCYIN",
  "categoryId": "pork",
  "emoji": "🐷",
  "source": "manual_demo_import",
  "updatedAt": 1710000000000
}
```

补充说明：
- `imgFileId` 用来存云存储文件标识，这是主字段。
- `img` 可以留空，后续小程序展示时再通过云接口换成临时访问地址，或者直接根据你最终实现策略补进去。
- `xhsLink` 不要保存整段“复制后打开小红书查看笔记”的分享文案，只存真正要跳转/复制的那条链接。

### 为什么图片放云存储、数据放数据库
- 图片文件属于二进制资源，适合放云存储。
- 菜名、分类、链接、图片 fileID 属于结构化字段，适合放数据库。
- 这样后续你要改菜名、换图片、换链接时，不需要重新导入整份 markdown。

### 如果后续要批量导入多个 Demo
建议把每个菜先整理成统一 JSON，而不是继续用自由格式 markdown。建议中间格式如下：

```json
{
  "dishId": "dish_001",
  "name": "农家小炒肉",
  "localImagePath": "Demo/Weixin Image_20260310204625_15_75.jpg",
  "xhsLink": "http://xhslink.com/o/9XzlPQqCYIN",
  "categoryId": "pork"
}
```

后续批量导入流程应为：
1. 本地读取 JSON
2. 逐条上传图片到云存储
3. 拿到每条图片的 `fileID`
4. 再把对应元数据写入 `dish_library`

不要反过来先写数据库再补图片，否则很容易出现数据库里有记录、但图片还没上传成功的脏数据。

### Demo 场景下推荐增加的字段
| 字段 | 是否建议 | 说明 |
| --- | --- | --- |
| `dishId` | 是 | 系统内部稳定 id |
| `name` | 是 | 菜名 |
| `imgFileId` | 是 | 云存储文件 ID |
| `img` | 建议 | 临时 URL 或后续展示地址 |
| `xhsLink` | 是 | 清洗后的小红书链接 |
| `source` | 建议 | 标记来源，如 `manual_demo_import` |
| `rawShareText` | 可选 | 保留原始分享文案，便于以后重新解析 |
| `updatedAt` | 是 | 更新时间 |

### 在你当前项目里的推荐落法
- `dish_library`：存“农家小炒肉”的主数据
- `meal_history`：先不要因为有 Demo 就写历史；只有用户真的把它加入某一天菜单时，再写历史记录
- `users`：只有用户登录后才需要

### 这条需求的直接结论
- `Demo` 里的图片应该上传到**云存储**
- `Demo` 里的菜名和小红书链接应该写到**云数据库**
- `Demo/demo.md` 本身只是导入素材，不是最终线上存储结构
- 如果现在只是试一条，最省事的方式是：**先手工上传图片，再手工写一条 `dish_library` 文档**

### 关键风险
- 小红书链接未必能在小程序里直接打开，这是本次需求最大的不确定项。
- 云开发登录成功不等于云同步一定成功，必须把这两步拆开处理。
- 如果历史只存 `dishId`，后续改菜名或图片时，过去记录会失真。
- 如果第一版就把候选池、抽奖、偏好、历史全部同时上云，极容易把需求做散。
- 如果菜品图片持续增加，本地主包很容易先碰到 2M 限制；而 `tabBar` 页面必须在主包内，这意味着“我的”页登录能力本身不能靠挪分包解决，必须提前做资源预算。
