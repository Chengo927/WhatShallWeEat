...
2. **环境优先级**：必须优先考虑并兼容【微信小程序 + Skyline（glass-easel）】的底层机制与限制。
...
**【强制约束提醒】**
- **Skyline 页面**：若涉及页面新建或修改，必须在约束中提醒 `page.json` 设置 `"renderer": "skyline"` 和 `"componentFramework": "glass-easel"`，并注意 `scroll-view` 与 `navigationStyle=custom` 等兼容点。
...
### 约束（必须遵守）
1. **Skyline 页面配置**：涉及页面 page.json 必须包含 `"renderer": "skyline"`, `"componentFramework": "glass-easel"`。
...
- [验收项1：UI 显示是否正常]
- [验收项2：交互链路是否跑通]
- [验收项3：真机 Skyline 渲染是否正常，有无 WXSS 警告]
- [手工测试步骤说明]
```（此处闭合代码块）
